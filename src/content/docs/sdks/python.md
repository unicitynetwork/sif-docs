...
Matches the wire shape defined by [`GuardResponse`](https://github.com/unicitynetwork/semanticd/blob/main/crates/semd-core/src/types/response.rs) on the server. Fields the server omits via `serde(skip_serializing_if)` are exposed in Python as `None` or the empty type.
