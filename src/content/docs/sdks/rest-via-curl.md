...
const result = await fetch("https://gateway.example.com/api/v1/guard", {
  method: "POST",
  headers: {
    "Authorization": "Bearer ps_your_key",
...
req.Header.Set("Authorization", "Bearer ps_your_key")
...
const ws = new WebSocket(
  "wss://gateway.example.com/ws/events?api_key=ps_your_key&types=verdict&actions=block",
);
ws.onmessage = (msg) => {
...
curl -sS "https://gateway.example.com/manage/audit/entries?action=block&page_size=50" \
  -H "Authorization: Bearer ps_admin_key" \
